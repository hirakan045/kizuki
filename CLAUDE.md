...
| **functional-design.md**      | 機能設計書。画面構成、コンポーネント設計、データフロー、ワイヤフレーム                 | ✅ 作成済 |
...
| 項目               | ルール                                                         |
| ------------------ | -------------------------------------------------------------- |
| ブランチ名         | `feature/` `fix/` `docs/` `refactor/` `chore/` + 対象          |
| 粒度               | **WBS の中項目単位**（例: `feature/screens` = 4章全体）        |
| コミットメッセージ | Conventional Commits（`feat:` `fix:` `docs:` 等）              |
| マージ             | **Squash merge**                                               |
| マージの判断       | **利用者が GitHub 上で差分を確認してから。自動でマージしない** |

詳細は `docs/development-guidelines.md` の9章を参照。

---

## セッション管理（DEVLOG.md）

⚠️ **週7時間・細切れの開発を前提とする。**セッションをまたいでコンテキストは失われるため、記録で引き継ぐ。

### セッション開始時

1. **`DEVLOG.md` の最新エントリを読む**（前回どこで終わったか、何が未解決か）
2. `WBS.md` で次のタスクを確認する

### セッション終了時

**`DEVLOG.md` の先頭に新しいエントリを追加する。**
...
⚠️ **「何をしたか」より「何に詰まって、どう判断したか」を残す。**
前者はコミット履歴を見れば分かる。後者は記録しないと失われる。

💡 この記録は面接・READMEで「なぜそうしたか」を語る材料にもなる。
...
⚠️ **main への直接コミットは行わない。**

```bash
git switch -c feature/[機能名]
```

粒度は **WBS の中項目単位**（例: `feature/screens` = 4章全体）。
...
- 概要（1〜2行）
- 対応するWBSのタスク番号
- 確認したこと（実機確認、型チェック・Lintの結果）
- 判断したこと（設計上の選択があれば）

⚠️ **PRを作成したら、そこで一旦停止して確認を求める。**
マージは利用者が差分を確認してから行う。自動でマージしない。

#### 10. DEVLOG更新（セッション終了時）
